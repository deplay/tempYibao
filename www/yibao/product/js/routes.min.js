angular.module("routes",[]).config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){function loadCss(css){return["$q","$rootScope",function($q,$rootScope){console.log("执行");
var dependencies=[];if(Array.isArray(css)){dependencies=css}else{dependencies.push(css)
}var proArr=[];angular.forEach(dependencies,function(v,k,o){var deferred=$q.defer();
var el=document.createElement("link");el.onload=el.onreadystatechange=function(e){if(el.readyState&&!/^c|loade/.test(el.readyState)){return
...
});return deferred.promise}]}$stateProvider.state("productList",{url:"/productList",templateProvider:loadTpl(["\\html\\product\\productList.min.html"]),resolve:{js:asyncLoad(["controller/product/productListCtrl.min"]),css:loadCss("\\css\\product\\productList.min.css")}})
}]);